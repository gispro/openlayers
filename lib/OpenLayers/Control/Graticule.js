...
OpenLayers.Control.Graticule = OpenLayers.Class(OpenLayers.Control, {
    ...
    */
    ...
    update: function() {
        ...
        // hand made transform
        /*
        var maxExtent = this.map.getMaxExtent().clone();
        
        //maxExtent.transform(mapProj, llProj);
        //OpenLayers.Projection.transform(maxExtent, mapProj, llProj);
        
        
        var leftTop = new OpenLayers.LonLat// OpenLayers.Geometry.Point
        (maxExtent.left,//-maxExtent.centerLonLat.lon, 
        maxExtent.top);//-maxExtent.centerLonLat.lat);
        var rightBottom = new OpenLayers.LonLat//OpenLayers.Geometry.Point
        (maxExtent.right,//+maxExtent.centerLonLat.lon, 
        maxExtent.bottom);//+maxExtent.centerLonLat.lat);
        //OpenLayers.Projection.transform(leftTop, mapProj, llProj);
        //OpenLayers.Projection.transform(rightBottom, mapProj, llProj);
        leftTop.transform(mapProj, llProj);
        rightBottom.transform(mapProj, llProj);
        */
        ...
        for(var i=0; i < centerLatPoints.length; ++i) {
            ...
            var pointList = [];
            var labelPoint = null;
            var latEnd = Math.min(centerLonPoints[0].y, 85);//maxExtent.top);// leftTop.lat);
            var latStart = Math.max(centerLonPoints[centerLonPoints.length - 1].y, -85);//maxExtent.bottom);//rightBottom.lat);
        ...
        for (var j=0; j < centerLonPoints.length; ++j) {
            ...
            var pointList = [];
            var lonStart = Math.min(centerLatPoints[0].x, -175); //centerLatPoints[0].x;
            var lonEnd = Math.min(centerLatPoints[centerLatPoints.length - 1].x, 175);
